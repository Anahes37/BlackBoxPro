...
fun localProp(key: String) = localProps.getProperty(key)
    ?: error("Missing property '$key' in mod/1.21.4/gradle.properties")

evaluationDependsOn(":common")
evaluationDependsOn(":1.21.4:runtime")

val commonSourceSet = project(":common").extensions.getByType(SourceSetContainer::class.java).getByName("main")
val runtimeSourceSet = project(":1.21.4:runtime").extensions.getByType(SourceSetContainer::class.java).getByName("main")
...
dependencies {
    ...
    implementation(project(":1.21.4:runtime"))
...
tasks.named<Jar>("jar") {
    // 1.21.4 的 runtime 工程已经内嵌了 common 源码/输出，这里避免再次打入导致重复条目
